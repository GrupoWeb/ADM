...
public class FactorajeAction extends RugBaseAction {
	...
	public String creaInscripcion() {
		...
		MyLogger.Logger.log(Level.INFO, "RUG FACTORAJES creaInscripcion");
		try {
			...
			if (getAcreedorTO() != null) {
				...
				if (idTramite.intValue() != 0) {
					...
					if (actbit) {
						...
						throw new Exception(
								"RUG-InscripcionAction>>CreaInscripcion:::no se pudo actualizar la bitacora del tramite");
					}
				} else {
					throw new Exception(
							"RUG-InscripcionAction>>CreaInscripcion:::no se pudo crear el tramite de inscripcion");
				}
			} else {
	...
	public String saveformulario() {
		String regresa = Constants.FAILED;
		MyLogger.Logger.log(Level.INFO, "RUG FACTORAJES saveFormulario");
		try {
			if (inscripcionTO == null) {
				inscripcionTO = new InscripcionTO();
			}
			...
			if (idTramite != null && idTramite != 0) {
				...
				setListaOtorgantes(inscripcionService
						.getOtorgantesByTramite((Integer) sessionMap.get(Constants.ID_TRAMITE_NUEVO)));
				setListaDeudores(
						inscripcionService.getDeudoresByTramite((Integer) sessionMap.get(Constants.ID_TRAMITE_NUEVO)));
				setListaAcreedores(inscripcionService
						.getAcreedoresByTramite((Integer) sessionMap.get(Constants.ID_TRAMITE_NUEVO)));
				...
				if (actbit) {
					/** emulo paso 3 **/
					boolean actbit2 = inscripcionService.insertBitacoraTramite(Integer.valueOf(idTramite), 1, 3, null,
							"V");
					if (actbit2) {
						/** emulo la vigencia **/
						...
						if (sePudo) {
							...
							regresa = Constants.SUCCESS;
							setEsFactoraje("factoraje");
						} else {
							throw new Exception(
									"RUG-InscripcionAction>>actualizaVigencia:::no se pudo actualizar la bitacora de esta inscripcion");
						}
					...
					throw new Exception(
							"RUG-InscripcionAction>>agregarGarantia::: no se logro actualizar la bitacora de esta inscripcion");
				}
			} else {
