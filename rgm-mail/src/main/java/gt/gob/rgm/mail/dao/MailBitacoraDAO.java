...
public class MailBitacoraDAO {
	...
	List<MailTO> sendToMail = new ArrayList<>();
...
//             psTxResult       OUT  VARCHAR2
	...
	public List<MailTO> getMailById(int idMail)
	{
		try
		{
			String sql = "SELECT ID_MAIL, DESTINATARIO,DESTINATARIO_CC,DESTINATARIO_CCO,ASUNTO,MENSAJE ,SMTP_HOST,SMTP_PORT,SMTP_USER_MAIL,SMTP_PASSWORD,SMTP_AUTH ,SMTP_SSL_ENABLE,MAIL_CONTENT_TYPE from V_MAIL_POOL_PENDIENTE WHERE ID_MAIL = ?";
			...
			rs = ps.executeQuery();
			while(rs.next()){
				MailTO resultMail= new MailTO();
				resultMail.setIdMail(rs.getInt("ID_MAIL"));
				...
				messageTO.setMailSubject(rs.getString("ASUNTO"));
				messageTO.setMailText(rs.getString("MENSAJE"));
				resultMail.setMessageTO(messageTO);
				...
				accountMailTO.setSmtpSslEnable(rs.getString("SMTP_SSL_ENABLE"));
				accountMailTO.setMailContentType(rs.getString("MAIL_CONTENT_TYPE"));
				resultMail.setAccountMailTO(accountMailTO);

				sendToMail.add(resultMail);
